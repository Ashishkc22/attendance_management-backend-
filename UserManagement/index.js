...
app.get("/ping", (req, res, next) => {
  try {
    res.status(200).json({ message: "Pong from user management." });
  } catch (error) {
    next(error);
  }
});
...
const { errorHandling, authMiddleware } = require("./src/middleware");

app.use("/api", authMiddleware, require("./src/controllers/external"));
